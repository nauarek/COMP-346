 ...
 * @author Kerly Titus
 */
public class NetworkV2 extends Thread {

    private static int maxNbPackets;                           /* Maximum number of simultaneous TransactionsV2 handled by the network buffer */
    ...
    private static TransactionsV2 inComingPacket[];              /* Incoming network buffer */
    private static TransactionsV2 outGoingPacket[];              /* Outgoing network buffer */
    ...
    NetworkV2() {
        ...
        portID = 0;
        maxNbPackets = 10;
        inComingPacket = new TransactionsV2[maxNbPackets];
        outGoingPacket = new TransactionsV2[maxNbPackets];
        for (i=0; i < maxNbPackets; i++) {
            inComingPacket[i] = new TransactionsV2();
            outGoingPacket[i] = new TransactionsV2();
        }
    ...
    /**
     *  Transmitting the TransactionsV2 from the client to the server through the network
     *
     * @return
     * @param inPacket transaction transferred from the client
     *
     */
    public static boolean send(TransactionsV2 inPacket) {
        try {
            inBuffEmpty.acquire();
            mutexIn.acquire();
        }
    ...
    /** Transmitting the TransactionsV2 from the server to the client through the network
     * @return
     * @param outPacket updated transaction received by the client
     *
     */
    public static boolean receive(TransactionsV2 outPacket) {
        try {
            outBuffFull.acquire();
            mutexOut.acquire();
        }
    ...
    /**
     *  Transferring the completed TransactionsV2 from the server to the network buffer
     *
     * @return
     * @param outPacket updated transaction transferred by the server to the network output buffer
     *
     */
    public static boolean transferOut(TransactionsV2 outPacket) {
        try {
    ...
    /**
     *  Transferring the TransactionsV2 from the network buffer to the server
     * @return
     * @param inPacket transaction transferred from the input buffer to the server 
     *
     */
    public static boolean transferIn(TransactionsV2 inPacket) {
        try {
            inBuffFull.acquire();
            mutexIn.acquire();
        }
