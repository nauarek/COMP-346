...
public class ServerV2 extends Thread {
	...
	private static AccountsV2 [] account;              		/* Accounts to be accessed or updated */
     ...
     * @param stid
     */
    ServerV2(String stid)
    {
    	if ( !(NetworkV2.getServerConnectionStatus().equals("connected")))
    	{
    		...
    		account = new AccountsV2[maxNbAccounts];
    		System.out.println("\n Inializing the Accounts database ...");
    		initializeAccounts( );
    		System.out.println("\n Connecting server to network ...");
    		if (!(NetworkV2.connect(NetworkV2.getServerIP())))
    		{
     ...
     * @param trans
     */
     public boolean processTransactions(TransactionsV2 trans)
     {   int accIndex;             	/* Index position of account to update */
         ...
         while ((!NetworkV2.getClientConnectionStatus().equals("disconnected")))
         {
        	 while ( (NetworkV2.getInBufferStatus().equals("empty") && !NetworkV2.getClientConnectionStatus().equals("disconnected")) )
        	 {
        		 Thread.yield(); 	/* Yield the cpu if the network input buffer is empty */
        	 }
        	 
        	 if (!NetworkV2.getInBufferStatus().equals("empty"))
        	 { 
        		 /* System.out.println("\n DEBUG : Server.processTransactions() - transferring in account " + trans.getAccountNumber()); */
        		 
        		 NetworkV2.transferIn(trans);                              /* Transfer a transaction from the network input buffer */
        		 ...
        		 while (NetworkV2.getOutBufferStatus().equals("full"))
        		 {
        		 ...
        		 NetworkV2.transferOut(trans);                            		/* Transfer a completed transaction from the server to the network output buffer */
     ...
     */
     ...
     {	
    	 return ("\n server IP " + NetworkV2.getServerIP() + "connection status " + NetworkV2.getServerConnectionStatus() + "Number of accounts " + getNumberOfAccounts());
     }
    ...
    public void run()
    {   TransactionsV2 trans = new TransactionsV2();
    	 long serverStartTime, serverEndTime;
    
	/* System.out.println("\n DEBUG : Server.run() - starting server thread " + getServerThreadId() + " " + Network.getServerConnectionStatus()); */

	    System.out.println("\n DEBUG : Server.run() - starting server thread " + NetworkV2.getServerConnectionStatus());
        ...
        if (getServerThreadId().equals("Server1")) {
            ...
            System.out.println("Terminating Server Thread1 - Running time " + (serverEndTime - serverStartTime) + "ms");
        ...
        if (getServerThreadRunningStatus1().equals("terminated") && getServerThreadRunningStatus2().equals("terminated")) {
            NetworkV2.disconnect(NetworkV2.getServerIP());
        }
